(function() {
  ...
  _.reject = function(collection, test) {
    ...
    var myArr = collection.slice();
    var toRemove = _.filter(myArr,test);
    var result = [];
    for (var i = 0; i < myArr.length; i ++){
      if (!toRemove.includes(myArr[i])){
        result.push(myArr[i]);
      }
    }
    return result;
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(itemSum, item) {
        return Boolean(iterator(item)) && itemSum;
    }, true);
  };
  ...
  _.delay = function(func, wait) {
    var sliceArg = Array.prototype.slice.call(arguments,2);
    return setTimeout(function(){func.apply(this,sliceArg)}, wait);
  };
